...
const AddQuestions = () => {
  ...
  const handleImageUpload = (event) => {
    ...
    // Determine the number of options based on conditions
    useEffect(() => {
      const questionType = "Not using this yet for true and false";
      let optionCount = 4; // Default option count
  
      if (selectedPaper.subcategorylvl2 === "Hogan Personality Inventory") {
        optionCount = 5;
      } else if (questionType === "True and False") {
        optionCount = 2;
      }
  
      // Adjust the options array
      setOptions((prevOptions) =>
        Array.from({ length: optionCount }, (_, i) => prevOptions[i] || "")
      );
    }, [selectedPaper.subcategorylvl2]);
//    }, [selectedPaper.subcategorylvl2, questionType]);
