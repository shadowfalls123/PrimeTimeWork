...
import { requireLoggedInUser } from "./utils/middlewares";
import { zipData, unzipData } from "./utils/zipUnzip";
import { TABLE_NAMES, assetBucket, PAYMENT_CONFIG } from "./utils/environment";
import { tableNamePrefix } from "./utils/getTableNamePrefix";


// import { verifyUserPurchase } from "./common_functions/verifyUserPurchase";
import { getTopTenPackagesFromS3 } from "./common_functions/getTopTenPackages";
...
import { updateTopTenPaperCache } from "./common_functions/updateTopTenPaperCache";
import { contactUsMsgs } from "./common_functions/ContactUs/contactUsMsgs";
import { getQuestionsCount } from "./common_functions/getQuestionsCount";
import { searchPacks } from "./common_functions/Search/searchPacks";
import { getpackpapdtls } from "./common_functions/getpackpapdtls";
import { fetchUserProfile, createUserProfile, updateUserProfileImage, deleteUserProfileImage, updateUserProfile } from "./common_functions/manageUser";
import { getUserFeedback } from "./common_functions/getUserFeedback";


import { createPackage } from "./tutor/createPackage";
import { createExam } from "./tutor/createExam";
import { publishPack } from "./tutor/publishPack";
import { updatePack } from "./tutor/updatePack";
import { updateExam } from "./tutor/updateExam";
import { updateQuestion } from "./tutor/updateQuestion";
// import { fetchAndReplaceImages } from "./tutor/fetchAndReplaceImages";
import { publishExam } from "./tutor/publishExam";
import { addQuestion } from "./tutor/addQuestion";
import { uploadQuestionsFile } from "./tutor/uploadQuestionsFile";
import { deleteQuestion } from "./tutor/deleteQuestion";
import { getSubmittedPapersForUser } from "./tutor/getSubmittedPapersForUser";
...
import { getSPQuestions, getSPQuestionsReviewAnswers } from "./tutor/manageSPExamQuestions";
import { fetchQuestionImage } from "./tutor/fetchQuestionImage";


// import { addMyCourse } from "./student/addMyCourse";
// import { getPaperDetailsFromS3 } from "./student/getPaperDetailsFromS3";
import { getMyCoursesForUserFromS3 } from "./student/getMyCoursesForUserFromS3";
import { saveUserFeedback } from "./student/saveUserFeedback";
import { getMyLearningPacksFromS3 } from "./student/getMyLearningPacksFromS3";
import { getMyLearningPacksFromDB } from "./student/getMyLearningPacksFromDB";
import { getPaperDetails } from "./student/getPaperDetails";
import { getExamQuestions } from "./student/getExamQuestions";
import { saveResultBatch } from "./student/saveResultBatch";
import { getMyExamReview } from "./student/getMyExamReview";
// import { getAnswersforPaper } from "./student/getAnswersforPaper";
import { getPackCoursesForUserFromS3 } from "./student/getPackCoursesForUserFromS3";


import { getMyCreditsParent } from "./purchase/payment/wallet/getMyCreditsParent";
import { razorpayCreateOrder } from "./purchase/payment/razorpay/razorpayCreateOrder";
import { phonepeCreateOrder } from "./purchase/payment/phonepe/phonepeCreateOrder";
import { confirmRazorpayPayment } from "./purchase/payment/razorpay/razorpaypayment";
import { addMyCoursesForUser } from "./purchase/addMyCoursesForUser";
...
if (!PAYMENT_CONFIG.razorpay.razorPayKeyId || !PAYMENT_CONFIG.razorpay.razorPayKeySecret || !PAYMENT_CONFIG.openAPI.openAPIKeyId || !PAYMENT_CONFIG.openAPI.openAPIKeySecret) {
...
//   apiKey: PAYMENT_CONFIG.openAPI.openAPIKeySecret
// });
/** 
...
const updateQuestionPartiQL = async (request, response) => {
  ...
  try {
    ...
    console.log("[updateQuestionPartiQL] UpdateItem succeeded:", JSON.stringify(result));
...
const getExamQuestionsChildForTutorFromDB = async (paperid) => {
  ...
  console.log("[getExamQuestionsChildForTutorFromDB] params 3.0 -> ", params);
  const results = await dynamoDBExam.query(params).promise();
...
const verifyTutorPackRel = async (userid, packid, paperid) => {
  try {
    ...
    const results = await dynamoDBExam.query(params).promise();
    console.log("results in verifyTutorPackRel 2.0 -> ", results);
    ...
    if (results.Items.length > 0) {
      ...
      console.log("packagePapers in verifyTutorPackRel 2.0 -> ", packagePapers);
      // Check if the paperid is in the packagePapers array
      const paperidIncluded = packagePapers.includes(paperid);
      console.log(
        "paperidIncluded in verifyTutorPackRel 2.0 -> ",
        paperidIncluded
      );
...
**/
//## getExamQuestionsChildFromDB May not be getting used
...
const getExamQuestionsChildFromDB = async (paperid) => {
  ...
  console.log("[getExamQuestionsChildFromDB] params 3.0 -> ", params);
  const results = await dynamoDBExam.query(params).promise();
  if (results.Items && results.Items.length > 0) {
    // Obfuscate column names before sending to frontend
    const obfuscatedResults = results.Items.map((item) =>
