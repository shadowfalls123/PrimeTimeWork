...
//------------------------------------------------------------------------
/*
const requireLoggedInUser = (request, response) => {
  ...
  if (userid) {
    ...
    // User is not logged in, send a response indicating unauthorized access
...
const updateQuestion = async (request, response) => {
  ...
  const params = {
    ...
    // KeyConditionExpression: "paperid = :paperid AND qid = :qid",
    // ExpressionAttributeValues: {
    ...
    UpdateExpression:
      "SET qt = :qt, o1 = :o1, o2 = :o2, o3 = :o3, o4 = :o4, a = :a, qe = :qe, examsection = :examsection, qmarks = :qmarks, negmarks = :negmarks",
    ExpressionAttributeValues: {
      // ":paperid": updatedRecord.pid,
      // ":qid": updatedRecord.quesid,
      ":qt": updatedRecord.question,
...
//           const imageUrl = `data:image/jpeg;base64,${utf8Image}`;

//           // const base64Image = imageData.Body.toString('base64');
//           // const imageUrl = `data:image/jpeg;base64,${base64Image}`;
          
//           // Map questionId to image data
//           imageMap[questionId] = imageUrl;
//       }
...
//           // Replace question image placeholder
//           if (imageMap[quesid] && qtxt.includes("{questionImage}")) {
//               question.qtxt = qtxt.replace("{questionImage}", `<img src="${imageMap[quesid]}" />`);
//           }
...
//               if (question[optionKey] && question[optionKey].includes(optionPlaceholder) && imageMap[quesid]) {
//                   question[optionKey] = question[optionKey].replace(optionPlaceholder, `<img src="${imageMap[quesid]}" />`);
//               }
//           }
...
const fetchAndReplaceImages = async (paperId, questionsData) => {
  ...
  try {
    ...
    const updatedQuestionsData = questionsData.map(question => {
//      const { quesid, qtxt, op1, op2, op3, op4 } = question;
      const { quesid, qtxt, op1, op2, op3, op4, explanation } = question;
...
/*
// Publish Exam - working code without Zip option
const publishExam = async (request, response) => {
  console.log("Inside updateQuestion request ", request);
  const publishExamData = JSON.parse(request.event.body);
  console.log("Inside updateQuestion publishExamData ", publishExamData);
  ...
  // Check if the paper with the same paper id exists
  const existingPaperIndex = existingData
    ? JSON.parse(existingData).findIndex(
        (existingPaper) => existingPaper.pid === paper.pid
      )
  ...
  const updatedPublishExamparams = {
    Bucket: assetBucket,
    Key: `submittedpapers/sp.txt`, // Use the key format used during upload
    Body: JSON.stringify(updatedExamArray),
    ContentType: "application/json",
  };
  console.log("Updated Publish Exam Params:", updatedPublishExamparams);

  try {
    await s3.putObject(updatedPublishExamparams).promise();
  } catch (error) {
    console.error("Error updating or creating the file:", error);
    return { statusCode: 500, body: "Error updating or creating the file" };
  }
  ...
  // Execute DynamoDB update
  try {
    console.log("update published status Params -> ", params);
    const result = await dynamoDBExam.update(params).promise();
    console.log("Exam Published Successfully:", JSON.stringify(result));
    return { statusCode: 200, body: "Exam Published Successfully." };
  } catch (err) {
...
const getExamQuestionsChildForTutorFromDB = async (paperid) => {
  ...
  const params = {
    ...
    ProjectionExpression:
      "paperid, qid, a, o1, o2, o3, o4, qt, qe, qmarks, examsection, negmarks, qimgpath, hasImage",
    //    Limit: count
    //    IndexName: `${tableNamePrefix}_GSI1`,
  };
...
const getPackCoursesForUserFromS3 = async (request, response) => {
  try {
    ...
    if (result.Items && result.Items.length > 0) {
      // //      const paperIds = result.Items.map((item) => item.paperid);
      //       const paperIds = result.Items.filter(item => item.itemCategory === 'paper').map(item => item.paperid);
      //       const packageIds = result.Items.filter(item => item.itemCategory === 'package').map(item => item.paperid);
...
const getMyLearningPacksFromS3 = async (request, response) => {
  try {
    ...
    // const params = {
    //   TableName: myCoursesTable,
    //   KeyConditionExpression: "userid = :userId AND itemCategory = :category",
    //   ExpressionAttributeValues: {
    //     ":userId": request.event.requestContext.authorizer.jwt.claims.username,
    //     ":category": "package",
    //   },
...
/*    
    // Save the individual question results
    const params = {
      RequestItems: {
        [examResultsTable]: updatedAnswers.map((record) => {
          // Map the column names to match the database structure
          const mappedRecord = {
            pqid: record.pid + "-" + record.quesid,
            qid: record.quesid,
            selectedAnswer: record.selectedAns,
...
const saveUserFeedback = async (request, response) => {
  try {
    ...
    /*
    // Save overall rating and number of responses to another file
    const metadataParams = {
      Bucket: assetBucket,
      Key: `userfeedback-metadata/${paperid}`, // Use a different key for metadata
      Body: JSON.stringify({ overallRating, numResponses }),
      ContentType: "application/json",
    };
...
const getAnswersforPaper = async (paperid) => {
  ...
  const params = {
    ...
    ProjectionExpression:
      "paperid, qid, a, o1, o2, o3, o4, qt, examsection, qmarks, negmarks",
    //    Limit: count
    //    IndexName: `${tableNamePrefix}_GSI1`,
  };
...
/*
//Check if user exists in database - This is working but in the new one returning the values differently
const getUserProfile = async (userid) => {
  const params = {
    ...
    ProjectionExpression: "fname, lname, countrycode, email, country" // Specify the columns to retrieve
  };
  console.log("dynamoDB getUserProfile Params -> ", params);
  try {
    const data = await dynamoDBExam.get(params).promise();
    return data.Item; // Return the user profile if found, or null if not found
  } catch (err) {
...
const getUserProfile = async (userid) => {
  ...
  try {
    ...
    try {
      ...
      // Add the S3 image data to the userProfile object
      userProfile.profileImage = s3Data.Body.toString("base64"); // Convert image data to base64
    } catch (s3Error) {
...
const fetchUserProfile = async (request, response) => {
  ...
  try {
    ...
    if (existingUserProfile) {
      // If user profile exists, skip record creation
      return { statusCode: 200, body: JSON.stringify(existingUserProfile) };
    } else {
      // If user profile doesn't exist, create a new profile
...
const updateUserProfileImage = async (request, response) => {
  ...
  try {
    ...
    //console.log("Inside updateUserProfileImage formData ", formData);
    // console.log("Inside updateUserProfileImage formData.firstname ", formData.firstname);
    // console.log("Inside updateUserProfileImage formData.profileImage ", formData.profileImage);
    const userid = request.event.requestContext.authorizer.jwt.claims.username;
...
const updateUserProfile = async (request, response) => {
  ...
  try {
    ...
    /*  let s3ImagePath = ''; // Declare s3ImagePath variable outside the conditional block
          ...
          // Check if the record has a profile image
          if (record.profileimage && record.profileimage instanceof Blob) {
            // Convert the Blob to a buffer
            const buffer = Buffer.from(await record.profileimage.arrayBuffer());
            ...
            const params = {
              ...
              Key: `profile/${userid}`, // Store in the 'profile' folder with the userid as the filename
              Body: buffer,
              ACL: 'private', // Set appropriate permissions -- // Restrict access to the uploaded image
              ContentType: 'image/jpeg', // Change content type according to your image type
            };
  ...
  */
    ...
    if (existingUserProfile) {
      ...
      // const updatedProfile = {
      //   ...existingUserProfile, // Retain the existing profile attributes
      //   fname: record.firstname || existingUserProfile.fname,
      //   lname: record.lastname || existingUserProfile.lname,
      //   countrynm: record.countryname,
      //   countrycd: record.countrycode,
      //   updatedt: currentdate,
      // };
...
const contactUsMsgs = async (request, response) => {
  ...
  // const currentMMYYYY = new Intl.DateTimeFormat('en-US', { month: '2-digit', year: 'numeric' }).format(currentdate);
...
router.add(
  ...
  async (request, response) => {
    if (requireLoggedInUser(request, response)) {
      try {
        const result = await getMyCoursesForUser(request, response);

        return {
          statusCode: 200,
          body: JSON.stringify(result) // Convert the result to JSON
        };
      } catch (error) {
        return {
          statusCode: 500,
          body: JSON.stringify({ message: 'CME501 - Internal server error' })
        };
      }
    }
  }
...
// //Get MyCourses for a user - Get users purchased courses
// router.add(
//   Matcher.HttpApiV2("GET", "/questions/getpackcourses/(:packPaperIDs)"),
